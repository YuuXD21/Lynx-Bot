...
function addRequest(from, request) {
	// Membaca data dari file request.json
	const requestData = fs.readFileSync('./request.json', 'utf8');
	let requests = [];
	
	// Mengubah data menjadi objek jika tidak kosong
	if (requestData) {
	...
	// Menulis kembali data ke file request.json
...
module.exports = async (sock, msg) => {
  ...
  const requestMessage = body.slice(9).trim();

  if (!requestMessage) {
    reply('Maaf, Anda harus memberikan deskripsi permintaan fitur.');
    return;
  }

  addRequest(user, requestMessage);

  reply('Terima kasih atas request fitur! Permintaan Anda telah disimpan.');
  break;
		...
		case 'stalktiktok':
			...
			break*/
		case 'ai':
			reply("Maaf sayangku, fitur ini sedang dikembangkan !")
...
╰───「 ${ownerName} 」`)
	break;
		...
		default:
			if (isCmd) {
				reply(`Maaf ya sayang, command *${prefix}${command}* tidak ada di list *${prefix}help*`)
			}
			break
	}
}
